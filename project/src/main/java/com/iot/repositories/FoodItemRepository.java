...
public interface FoodItemRepository extends JpaRepository<FoodItem, Long> {
    @Query("SELECT fi FROM FoodItem fi WHERE fi.deleted_at IS NULL AND fi.companyId = :company_id")
    ...
    @Query("SELECT fi FROM FoodItem fi WHERE fi.name LIKE %:keyword% AND fi.deleted_at IS NULL AND fi.companyId = :company_id")
    ...
    @Query("SELECT fi FROM FoodItem fi " +
            "WHERE fi.categoryId = :category_id AND fi.deleted_at IS NULL AND fi.companyId = :company_id")
